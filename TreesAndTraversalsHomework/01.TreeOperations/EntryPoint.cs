namespace TreeOperations
{
    ...
    public class EntryPoint
    {
        public static void Main()
        {
            ...
            var nodesTree = new Tree<int, int>();
            ...
            for (int i = 0; i < numberOfNodes - 1; i++)
            {
                ...
                nodesTree.AddNode(parentNodeInfo, parentNodeInfo, childNodeInfo, childNodeInfo);
            }

            var rootNode = nodesTree.RootNode;
            var leafs = nodesTree.GetAllLeafs();
            var middleNodes = nodesTree.GetAllMiddleNodes();
            int longestPath = nodesTree.GetLongestPath;

            Console.WriteLine("Root node value: {0}", rootNode.Value);
            Console.WriteLine("Leafs values: {0}", string.Join(", ", leafs));
